...
const NavBar = (props) => {
  ...
  return (
    <div className='relative flex  flex-nowrap  justify-start'>
      {/* Desktop Menu */}  
      <div id="desktopmenu" className="hidden md:flex text-gray-200  ">
        {links.map((menu, index) => {
          return (
            <MenuItem items={menu} key={uuid+index} path={menu.to} />
          )})}        
      </div>
              ...
              fixed -top-8 left-0 bg-black/70                  `}>
                <div className=' flex justify-end  w-full h-full ' >
                  <div 
                    ...
                    >
                      <AnimatePresence>
                        <div id='mainMenu' key='mainMenu' className=' flex justify-center flex-col   items-center min-h-[50vh]  '>
                        {links.map((menu,index) => {
                          return (
                          <MenuItem items={menu} key={index+uuid} index={index} uuid={uuid} mobile={true}/>
                          )})}
                        </div>
                        <div id='footMenu' key='footMenu' className=' flex justify-center  space-x-3 py-3 border-t border-white/30 '>
                          ...
                          <div className=' border-l border-white/30 ' />
                          <Link title='Home' href='/' data-umami-event="Home" className=' flex justify-center items-center p-2 ' >
                            <VipIcon className='md:w-8 md:h-8 w-6 h-6   duration-500  hover:scale-125' />
                          </Link>  
                        </div>
                        <Link id='github corner' key="githubcorner" title='GitHub' href={BLOG.githubUrl} data-umami-event="GITHUB"  >
                             <GithubIcon   /> 
                        </Link>
                      </AnimatePresence>
                  </div>
