...
export const ArticleLock = props => {
  ...
  const submitPassword = () => {
    ...
    if (!validPassword(p?.value)) {
      ...
      if (tips) {
        tips.innerHTML = ''
        tips.innerHTML = `<div class='text-red-500 animate__shakeX animate__animated'>${"Please check password."}</div>`
      }
    }
  }
    ...
    <div className='text-center space-y-3'>
      ...
      <div className='flex mx-4'>
        ...
        <div onClick={submitPassword} className="px-3 whitespace-nowrap cursor-pointer items-center justify-center py-2 bg-indigo-500 hover:bg-indigo-400 text-white rounded-r duration-300" >
          <i className={'duration-200 cursor-pointer fas fa-key'} >&nbsp;{"OK"}</i>
        </div>
      </div>
      <div id='tips'>
      </div>
